...
6. **Scaffold, then build the data objects.** Run `scaffold_data.py` (see
...
8. **Render both PDFs in one call.** Both `.data.json` files are already on disk
   ...
   letter in a **single** `render_pdf.py` invocation — the three path flags are
...
```
uv run --project ${CLAUDE_PLUGIN_ROOT} ${CLAUDE_PLUGIN_ROOT}/scripts/scaffold_data.py \
  --dir {dir} --source-dir {sourceDir} --lang <en|zh> --density <compact|standard>
```
...
When `--lang zh`:
- Pass `--lang zh` to `scaffold_data.py` — it sets `lang: "zh"` in both JSON files
