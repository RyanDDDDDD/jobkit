...
`applications/Testco/`. Machine artifacts (`resume.data.json`,
`cover_letter.data.json`) land in `applications/Testco/tmp/`; the PDFs and
`cover_letter.txt` are flat in `applications/Testco/`. There is no repo-level
`templates/` override in the fixture,
...
1. **All five artefacts exist.** `applications/Testco/tmp/resume.data.json`,
...
4. **`resume.pdf` renders within the page budget.** `render_pdf.py` prints an
...
6. **No forbidden technology.** Neither `tmp/resume.data.json` (raw text) nor the
...
7. **`cover_letter.pdf` exists** and `render_pdf.py` reports it as 1 page.
...
9. **`tmp/cover_letter.data.json` subject has no dangling reference.** The synthetic JD
...
10. **No university in the cover letter.** Neither `tmp/cover_letter.data.json` (raw
...
11. **No stray rendered HTML.** `--keep-html` was not passed, so `applications/Testco/`
    contains no `*.rendered.html` at any depth — in particular none in
    `applications/Testco/tmp/`.
...
12. **The `generate` report cites a `file:line` for every metric/skill claim.** The
    ...
    named skill used in `tmp/resume.data.json` / `tmp/cover_letter.data.json` is paired with
