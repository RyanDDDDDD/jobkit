...
# ---------------------------------------------------------------------------
# Placeholder value sets — one entry per {{...}} token the shipped templates
# actually contain (enumerate with:
...
# cover_letter.tex has no {{SUBJECT}} token — the subject line is assembled by the
# template from {{JOB_TITLE}} + {{REQ_ID}}. Fill both.
$coverFill = [ordered]@{
    '{{NAME}}'             = 'Sample Dev'
    '{{PHONE}}'            = '(+00) 000-000-000'
    '{{EMAIL}}'            = 'sample.dev@example.com'
...
$firstLine = Get-Content -LiteralPath $txt | Where-Object { $_.Trim() -ne '' } | Select-Object -First 1
Check "cover_letter.txt : produced"                       (Test-Path $txt)
